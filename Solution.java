public class Solution {
    ...
    public static int removeElement(int[] nums, int val) {
        ...
        var new_nums = new int[nums.length];
        var new_length = 0;

        // Filter unwanted elements, gather new range.
        for ( int i = 0; i < nums.length; i ++)
            if (nums[i] != val) {
                new_nums[new_length] = nums[i];
                new_length ++;
            }

        // Refill original array without unwanted elements,
        // Mark emptied indices with -1.
        for ( int i = 0; i < nums.length; i ++)
            nums[i] = i < new_length ? new_nums[i] : -1;
